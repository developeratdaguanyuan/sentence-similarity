...
class LSTM(object):
  def __init__(self, vocab_size, class_size, word_vectors,
    ...
    # LSTM
    z1, _ = self._apply_lstm(e1, self.hidden_dim, 'encoder')
    z2, _ = self._apply_lstm(e2, self.hidden_dim, 'encoder', True)
  ...
  def _apply_lstm(self, input, unit_num, scope=None, reuse=False):
    '''
    LSTM model
    ...
    with tf.variable_scope(scope_name, reuse=reuse):
      ...
      cell = tf.nn.rnn_cell.LSTMCell(unit_num, forget_bias=1.0, initializer=initializer, use_peepholes=True,
  ...
  def train(self, train_data_producer, valid_data_producer, num_epoch):
    ...
    :return:
    '''
    logging.basicConfig(filename='../logs/lstm.log', filemode='w', level=logging.INFO)
      ...
      for i in range(num_iteration):
        ...
        if i > 0 and i % (train_data_producer.size / self.batch) == 0:
          ...
          if valid_loss_t < valid_loss:
            ...
            save_path = saver.save(self.sess, "../save_models/lstm_" + str(i))
